...
test('creates canvas nodes with Liblib context interactions, persistence, drag, and 200% reachability', async ({
  ...
  await expect(nodeMenu.getByRole('menuitem')).toHaveCount(10)
...
test('edits and persists all specialized Liblib node detail panels', async ({ page }) => {
  ...
  await expect(audioPanel.getByText('00:12')).toBeVisible()
  await expect(audioPanel.getByRole('combobox', { name: '音色' })).toHaveValue('zh_female_vv_uranus_bigtts')
  await audioPanel.getByRole('slider', { name: '语速' }).press('ArrowRight')
  await audioPanel.getByRole('slider', { name: '语速' }).press('ArrowRight')
  await expect(audioPanel.getByRole('slider', { name: '语速' })).toHaveValue('1.2')
  await audioPanel.getByRole('slider', { name: '音量' }).press('ArrowRight')
  await expect(audioPanel.getByRole('slider', { name: '音量' })).toHaveValue('51')
  ...
  const directorObjectTree = directorPanel.getByRole('tree', { name: '3D对象树' })
  await expect(directorObjectTree.getByRole('treeitem', { name: '人形素模 01 人形素模' })).toBeVisible()
  await directorPanel.getByRole('button', { name: '添加立方体' }).click()
  await expect(directorObjectTree.getByRole('treeitem', { name: '立方体 01 立方体' })).toBeVisible()
  ...
  await directorPanel.getByRole('button', { name: '导出四视图 PNG 到画布' }).click()
  ...
  const persistedDirectorPanel = page.getByRole('region', { name: '导演台 01 导演台参数' })
  await expect(persistedDirectorPanel.getByRole('treeitem', { name: '主场景方桌 立方体' })).toBeVisible()
  await expect(persistedDirectorPanel.getByRole('button', { name: '顶部视图' })).toHaveAttribute('aria-pressed', 'true')
  expect(browserErrors).toEqual([])
})
...
test('shows only real image and video models with their supported parameters', async ({ page }) => {
  ...
  await expect(imageModel.locator('optgroup[label="待接入"]')).toHaveCount(0)
  await expect(imageModel).not.toContainText(/720全景|九宫格|四宫格|25宫格|光影|设定图/)
