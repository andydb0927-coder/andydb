...
test('creates canvas nodes with keyboard, persistence, drag, and 200% reachability', async ({
  ...
  const textNode = page.getByRole('button', { name: '文本 01' })
  ...
  const imageNode = page.getByRole('button', { name: '图片 01' })
  ...
  await expect(page.getByRole('button', { name: '文本 02' })).toBeHidden()
  ...
  const reloadedImage = page.getByRole('button', { name: '图片 01' })
  ...
  expect(
    await page
      ...
      .filter({ has: page.getByRole('button', { name: '文本 01' }) })
