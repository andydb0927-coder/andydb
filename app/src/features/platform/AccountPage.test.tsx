...
test('shows local membership matrix and persists a simulated upgrade', async () => {
  ...
  const permissions = screen.getByRole('table', { name: '本地协作角色权限' })
  expect(permissions).toHaveTextContent('所有者')
  expect(permissions).toHaveTextContent('编辑者')
  expect(permissions).toHaveTextContent('只读')
  expect(permissions).toHaveTextContent('管理成员')
