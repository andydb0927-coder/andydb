...
describe('platform task drawer', () => {
  ...
  test('shows all phases, aggregate progress, and navigable targets', () => {
    ...
    expect(screen.getByText(/当前阶段：/)).toHaveTextContent('当前阶段：工作流与模板')
  ...
  test('persists a status change and restores it on remount', async () => {
    ...
    await user.selectOptions(
      screen.getByRole('combobox', { name: '更新 工作流与模板 状态' }),
      'completed',
    )
    expect(screen.getByText('8 / 13 已完成')).toBeVisible()
    first.unmount()

    renderDrawer(storage)
    expect(screen.getByRole('combobox', { name: '更新 工作流与模板 状态' })).toHaveValue('completed')
  })
})
