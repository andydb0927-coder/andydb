...
import { SimilarSubjectsReview } from './SimilarSubjectsReview'
...
interface CreateSubjectDialogProps {
  ...
  onCancel(): void
  onFindSimilar?(value: CreateSubjectFormValue): Promise<SimilarSubject[]>
  onSubmit(value: CreateSubjectFormValue, mergeIntoId?: string): void
}
...
export function CreateSubjectDialog({
  ...
  const [review, setReview] = useState<{ value: CreateSubjectFormValue; candidates: SimilarSubject[] }>()
  const [checking, setChecking] = useState(false)
  const checkInFlight = useRef(false)
  const alive = useRef(true)
  useEffect(() => { alive.current = true; return () => { alive.current = false } }, [])
  useEffect(() => setReview(undefined), [name, description, tags, appearance, clothing])
  const dirtyFields = useRef(new Set<string>())
  const formRevision = useRef(0)
  const markEdited = (field: string) => { dirtyFields.current.add(field); formRevision.current++; setReview(undefined) }
  ...
  const submit = (event: FormEvent) => {
    ...
    if (!name.trim() || busy || checkInFlight.current || review) return
    extractionController.current?.abort()
    extractionController.current = undefined
    setExtracting(false)
    const value: CreateSubjectFormValue = {
      name: name.trim(),
    ...
    if (!onFindSimilar) { onSubmit(value); return }
    checkInFlight.current = true; setChecking(true); setExtractionError('')
    const submittedRevision = formRevision.current
    void onFindSimilar(value).then(candidates => {
      if (!alive.current) return
      if (submittedRevision !== formRevision.current) { setExtractionError('资料已变更，请重新保存以核对相似主体。'); return }
      if (candidates.length) setReview({ value, candidates })
      else onSubmit(value)
    }, () => { if (alive.current) setExtractionError('相似主体读取失败，请重试；未创建记录。') }).finally(() => { checkInFlight.current = false; if (alive.current) setChecking(false) })
  }
  ...
  return (
    <div className="canvas-dialog-backdrop" role="presentation">
      ...
      >
        ...
        <label>主体名称
          <input ref={nameRef} aria-label="主体名称" value={name} maxLength={80} onChange={(event) => { markEdited('name'); setName(event.target.value) }} />
        </label>
        <label>描述
          <textarea aria-label="主体描述" value={description} maxLength={400} onChange={(event) => { markEdited('description'); setDescription(event.target.value) }} />
        </label>
        <label>标签
          <input aria-label="主体标签" value={tags} placeholder="用逗号分隔，最多 8 个" onChange={(event) => { markEdited('tags'); setTags(event.target.value) }} />
        </label>
        {extracted ? <>
          <label>外貌<input aria-label="主体外貌" maxLength={180} value={appearance} onChange={event => { markEdited('appearance'); setAppearance(event.target.value) }} /></label>
          <label>服装<input aria-label="主体服装" maxLength={180} value={clothing} onChange={event => { markEdited('clothing'); setClothing(event.target.value) }} /></label>
        </> : null}
        <div className="create-subject-dialog__provider">
          <button type="button" disabled={busy || checking || Boolean(review) || extracting || Boolean(unavailable)} aria-label="AI 身份提取" title={unavailable ?? '重新提取会再次计费，仅补充未手动修改的字段。'} onClick={() => void extract()}>
        ...
        {review ? <SimilarSubjectsReview candidates={review.candidates} busy={busy} onCancel={() => setReview(undefined)} onCreate={() => onSubmit(review.value)} onMerge={id => onSubmit(review.value, id)} /> : null}
        <footer>
          <button type="button" disabled={busy} onClick={cancel}>取消</button>
          <button type="submit" disabled={busy || checking || Boolean(review) || !name.trim()}>{busy ? '保存中…' : checking ? '核对相似主体…' : '保存到主体库'}</button>
        </footer>
      </form>
    </div>
  )
